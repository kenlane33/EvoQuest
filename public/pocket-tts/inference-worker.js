...
async function openModelCache() {
    ...
    try {
        return await caches.open(modelCacheName);
    } catch {
        return null;
    }
}
...
async function loadOnnxSession(url, sessionOptions) {
...
async function loadBundle(language, { initialLoad = false } = {}) {
    ...
    const metadataResponse = await cachedFetch(bundlePath(language, "bundle.json"));
    ...
    const [encoderRes, textCondRes, flowMainRes, flowFlowRes, decoderRes] = await Promise.all([
        loadOnnxSession(bundlePath(language, MODEL_STEMS.mimi_encoder), sessionOptions),
        loadOnnxSession(bundlePath(language, MODEL_STEMS.text_conditioner), sessionOptions),
        loadOnnxSession(bundlePath(language, MODEL_STEMS.flow_lm_main), sessionOptions),
        loadOnnxSession(bundlePath(language, MODEL_STEMS.flow_lm_flow), sessionOptions),
        loadOnnxSession(bundlePath(language, MODEL_STEMS.mimi_decoder), sessionOptions),
    ]);
    ...
    const tokenizerResponse = await cachedFetch(bundlePath(language, bundleMetadata.tokenizer_file));
    ...
    if (bundleMetadata.bos_before_voice_file) {
        const bosResponse = await cachedFetch(bundlePath(language, bundleMetadata.bos_before_voice_file));
        if (bosResponse.ok) {
    ...
    const voicesResponse = await cachedFetch(bundlePath(language, "voices.bin"));
...
self.onmessage = async (e) => {
    ...
    try {
        ...
        if (type === "load") {
            ...
            await loadBundle(language, { initialLoad: true });
            return;
        }
