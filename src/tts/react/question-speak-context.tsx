...
import { canAutoReadAloud } from '../engine/read-aloud';
...
export function QuestionSpeakProvider({
  ...
  const { enabled, autoRead, voice, volume } = useReadAloudSettings();
  ...
  const speak = useCallback(
    ...
    [enabled, voice, volume, cancelActive],
  );
    ...
    const sequence =
      autoSequence && autoSequence.length > 0
        ...
        : autoSlot
          ? [autoSlot]
          : [];
    if (sequence.length === 0 || !autoKey || !canAutoReadAloud({ enabled, autoRead })) return;
