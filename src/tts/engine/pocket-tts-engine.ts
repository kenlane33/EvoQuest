...
import { POCKET_TTS_DEFAULT_VOICE, POCKET_TTS_EXTRA_VOICES } from './pocket-tts';
import { isPocketTtsAvailable } from './read-aloud';
import {
  POCKET_TTS_BUNDLE_BASE,
  POCKET_TTS_MODEL_CACHE_NAME,
} from './pocket-tts-model-cache';
import {
  progressForWorkerStatus,
  setReadAloudBootstrapProgress,
  startReadAloudBootstrap,
} from './read-aloud-bootstrap';
...
import {
  ...
  ttsTimelineEnabled,
  ttsWorkerMark,
} from './pocket-tts-timeline';
import { PCMPlayerWorklet } from './vendor/PCMPlayerWorklet.js';

const SAMPLE_RATE = 24_000;
const SPEECH_ONSET_THRESHOLD = 0.02;
const DEFAULT_LANGUAGE = getReadAloudConfig().defaultLanguage;
...
export class PocketTtsEngine {
  ...
  private async init(): Promise<void> {
    ...
    try {
      ...
      this.worker = new Worker(getReadAloudConfig().workerUrl, { type: 'module' });
